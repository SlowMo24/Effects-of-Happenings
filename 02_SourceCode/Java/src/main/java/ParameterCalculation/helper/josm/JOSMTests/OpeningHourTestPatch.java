...
/**
 * Tests the correct usage of the opening hour syntax of the tags {@code opening_hours},
 * {@code collection_times}, {@code service_times} according to
 * <a href="https://github.com/ypid/opening_hours.js">opening_hours.js</a>.
 *
 * @since 6370
 */
public class OpeningHourTestPatch extends Test.TagTest {
  ...
  public void initialize() throws Exception {
    super.initialize();
    if (ENGINE != null) {
      try (CachedFile cf = new CachedFile("resource://new_opening_hours.js");
      ...
      Logging.warn(
          "Unable to initialize OpeningHourTest because no JavaScript engine has been found");
    }
  }

  /**
   * In OSM, the syntax originally designed to describe opening hours, is now used to describe a few
   * other things as well. Some of those other tags work with points in time instead of time ranges.
   ...
   * <a href="https://github.com/ypid/opening_hours.js">opening_hours.js</a> and returns an object
   * on which methods can be called to extract information.
   *
   ...
   * @param mode whether to validate {@code value} as a time range, or points in time, or both. Can
   * be null
   * @param locale the locale code used for localizing messages
   * @return The value returned by the underlying method. Usually a
   * {@code jdk.nashorn.api.scripting.ScriptObjectMirror}
   * @throws ScriptException if an error occurs during invocation of the underlying method
   * @throws NoSuchMethodException if underlying method with given name or matching argument types
   * cannot be found
   * @since 13147
   */
  public Object parse(String value, String tagKey, CheckMode mode, String locale) throws
      ScriptException, NoSuchMethodException {
    return ((Invocable) ENGINE).invokeFunction("oh", value, tagKey, mode != null ? mode.code : -1,
        locale);
  }
  ...
  public class OpeningHoursTestError {
     ...
     * @return The real test error given to JOSM validator. Can be fixable or not if a prettified
     * values has been determined.
     */
    public TestError getTestError(final OsmPrimitive p, final String key) {
      final TestError.Builder error = TestError.builder(OpeningHourTestPatch.this, severity, 2901)
   ...
   * <a href="https://github.com/ypid/opening_hours.js">opening_hours.js</a> and returns a list
   * containing validation errors or an empty list. Null values result in an empty list.
   *
   * @param key the OSM key (should be "opening_hours", "collection_times" or "service_times"). Used
   * in error message
   * @param value the opening hour value to be checked.
  ...
  public List<OpeningHoursTestError> checkOpeningHourSyntax(final String key, final String value,
    ...
    try {
      ...
      try {
        ...
        Logging.warn("Error value was: " + value);
        errors.add(new OpeningHoursTestError("Could not parse OpeningHours.", Severity.ERROR,
            prettifiedValue));
        //Logging.warn(e);
      }
      for (final Object i : getOpeningHoursErrors(r)) {
        errors.add(new OpeningHoursTestError(getErrorMessage(key, i), Severity.ERROR,
            prettifiedValue));
      }
      for (final Object i : getOpeningHoursWarnings(r)) {
        errors.add(new OpeningHoursTestError(getErrorMessage(key, i), Severity.WARNING,
            prettifiedValue));
      }
      if (!ignoreOtherSeverity && errors.isEmpty() && prettifiedValue != null && !value.equals(
          prettifiedValue)) {
        errors.add(new OpeningHoursTestError(tr("opening_hours value can be prettified"),
            Severity.OTHER, prettifiedValue));
      }
   ...
   * @throws NoSuchMethodException if method "prettifyValue" or matching argument types cannot be
   * found
   * @throws ScriptException if an error occurs during invocation of the JavaScript method
   * @since 13296
   */
  public final String getOpeningHoursPrettifiedValues(Object r) throws NoSuchMethodException,
      ScriptException {
   ...
   * @since 13296
   */
  public final List<Object> getOpeningHoursErrors(Object r) throws NoSuchMethodException,
      ScriptException {
   ...
   * @throws NoSuchMethodException if method "getWarnings" or matching argument types cannot be
   * found
   * @throws ScriptException if an error occurs during invocation of the JavaScript method
   * @since 13296
   */
  public final List<Object> getOpeningHoursWarnings(Object r) throws NoSuchMethodException,
      ScriptException {
   ...
   *
   * @param o error/warning message returned by {@link #getOpeningHoursErrors} or
   * {@link #getOpeningHoursWarnings}
  ...
  public static String getErrorMessage(Object o) {
    return o.toString().trim()
        ...
        .replace("Unexpected token (school holiday parser):", tr(
            "Unexpected token (school holiday parser):"))
